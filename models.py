...
class DLFSRecModel(nn.Module):
    ...
    def forward(self, input_ids, input_context):  #input_ids:[256,50], input_context: [256,50,6]

        mean_id_emb = self.item_mean_embeddings(input_ids)  # [256,50,128]
        cov_id_emb = self.item_cov_embeddings(input_ids) # [256,50,128]

        input_attrs = self.args.items_feature[input_ids]   #[256, 50, 2326]
        mean_side_dense = self.side_mean_dense(torch.cat((input_context, input_attrs), dim=2))  #[256, 50, 128]
