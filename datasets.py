...
def get_pad(args,user_seq, max_len):
    ...
    if args.period_add_like_cxt==True: 
        for seq in tqdm.tqdm(user_seq):
            ids, cxt,period = [], [],[]
            for item_id, act_time,item_perioid in seq[:-1]:
                ids.append(item_id)
                cxt.append(act_time)
                period.append(item_perioid)  # add item period
            pad_num = max_len - len(ids)
            if pad_num == max_len:
                result.append([np.zeros(max_len, dtype=int), np.zeros((max_len, 6)), np.zeros(max_len)])
...
class DLFSRecDataset(Dataset):
    def __init__(self, args, user_seq, test_neg_items=None, data_type='train'):
        ...
        if args.period_add_like_cxt==True:  # add period
            if data_type == 'train':
                ...
                for user, seq in user_seq.items():
                    self.user_seq.append(seq[:-1]) # leave 1 out
            else:
                for user, seq in user_seq.items():
                    self.user_seq.append(seq) # all sequence
        else:  # baseline
            if data_type == 'train':
                for seq in user_seq:
                    input_ids = seq[-(self.max_len + 2):-2] # leave 2 out
    ...
    def __getitem__(self, index): # error
        ...
        if self.args.period_add_like_cxt==True:  #period add
            ...
            input_ids, input_cxt, input_period = self.inputs[index]

            assert len(input_ids) == self.max_len
            assert len(input_cxt) == self.max_len
            assert len(input_period)==self.max_len
            import IPython; IPython.embed(colors="Linux"); exit(1)

            pos_id, pos_cxt,pos_period = items[-1] # last
            seq_set = set([x[0] for x in items])
            neg_id = neg_sample(seq_set, self.args.item_size) # random item
            neg_cxt = pos_cxt  # neg_period, neg_cxt redefine
            ...
            inputs = {
                ...
                'period':torch.tensor(input_period, dtype=torch.long) # add period
            }
            pos = {
                'pos_id': torch.tensor(pos_id, dtype=torch.long),
                'pos_cxt': torch.tensor(pos_cxt, dtype=torch.float32),
                'pos_period':torch.tensor(input_period, dtype=torch.long) # add period
            }
            neg = {
                'neg_id': torch.tensor(neg_id, dtype=torch.long),
                'neg_cxt': torch.tensor(neg_cxt, dtype=torch.float32),
                'neg_period':torch.tensor(input_period, dtype=torch.long) # add period
            }
            ...
            neg_cxt = pos_cxt  # why????? pos_id<->neg_id
